...
void write_array(std::ostream &out, std::vector<std::array<T, N> > arrays,
                 ...
                 const std::string &col_separator = " ");
template <class T, size_t N>
void write_array(const std::string &fname,
                 std::vector<std::array<T, N> > arrays, const size_t ncols = 1,
                 const std::string &col_separator = " ");
}
...
public:
  io_array_read_error(const std::string &msg =
                          "Failed to read array from file.",
                      const std::string &type = "io_array_read_error");
};
class io_array_write_error : public io_write_error {
public:
  io_array_write_error(const std::string &msg =
                           "Failed to write array to file.",
...
template <class T, size_t N>
void write_array(const std::string &fname,
                 std::vector<std::array<T, N> > arrays, const size_t ncols,
                 const std::string &col_separator) {
  std::ofstream f{ fname };
  for (const auto &a : arrays) {
    write_array(f, a, ncols, col_separator);
