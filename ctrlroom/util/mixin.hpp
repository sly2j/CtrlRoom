...
template <class T> class comparison_mixin {
public:
  bool operator!=(const T& rhs) const { return !(derived() == rhs); }
...
// operators ++/--
template <class T> class postfix_mixin {
public:
  T operator++(int) {
    T tmp{derived()};
    ++derived();
...
template <class T> class add_subtract_mixin {
public:
  using difference_type = ptrdiff_t;
  T& operator++() { return derived() += 1; }
  T& operator--() { return derived() += -1; }
  T& operator-=(const difference_type n) { return derived() += -n; }
  T operator+(const difference_type n) const {
    T tmp{derived()};
    tmp += n;
    return tmp;
  }
  T operator-(const difference_type n) const { return operator+(-n); }
