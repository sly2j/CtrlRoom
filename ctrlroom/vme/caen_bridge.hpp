...
#define HANDLE_CAEN_ERROR(err, msg)                                            \
  if ((err)) {                                                                 \
    throw decode_error((err), (msg));                                          \
  }

namespace ctrlroom {
namespace vme {

class caen_bridge : public vme::master<caen_bridge> {
public:
...
inline void caen_bridge::wait_for_irq() const { wait_for_irq(timeout_); }
